        ...
        class << self
          def filter(controller)
            raise "Cannot use the CASClient filter because it has not yet been configured." if config.nil?
             ...
             if controller.request.url =~ /json/
               controller.logger.info "Detected Json request. By-passing CAS"
               return true
             end

            if @@fake_user
              controller.logger.info "Detected test environment. By-passing CAS"
              controller.session[client.username_session_key] = @@fake_user
              controller.session[:cas_extra_attributes] = {:id => 1}
              return true
            end
